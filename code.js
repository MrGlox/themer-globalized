...
function countNewThemes() {
  var themes = __spread(
    new Set(
      cleanedStyleData.map(function (style) {
        return style.theme;
      }),
    ),
  );
...
function mergeNewThemesWithExisting() {
  cleanedStyleData = removeDuplicatesBy(function (style) {
    return style.key;
  }, collectedStyleData);
  if (cleanedStyleData) {
...
function applyTheme(applyTo) {
  ...
  if (nodes) {
    ...
    var colorStyles = __spread(
      new Set(
        jsonBinData.map(function (style) {
          return style.theme === selectedTheme && style.type === 'PAINT';
        }),
      ),
    );
    var textStyles = __spread(
      new Set(
        jsonBinData.map(function (style) {
          return style.theme === selectedTheme && style.type === 'TEXT';
        }),
      ),
    );
    var effectStyles = __spread(
      new Set(
        jsonBinData.map(function (style) {
          return style.theme === selectedTheme && style.type === 'EFFECT';
        }),
      ),
    );
            ...
            return __generator(this, function (_a) {
              switch (_a.label) {
                case 0:
                  ...
                  return [
                    4 /*yield*/,
                    figma.importStyleByKeyAsync(newStyleKey),
                  ];
                case 1:
                  newStyle = _a.sent();
...
function findMatchInSelectedTheme(styleKey) {
  ...
  var currentStyle = jsonBinData.find(function (style) {
    return style.key === styleKey;
  });
  ...
  if (currentStyle) {
    ...
    var matchedStyle = jsonBinData.find(function (style) {
      return style.name === name_1 && style.theme === selectedTheme;
    });
    if (matchedStyle) {
      ...
      return __generator(this, function (_a) {
        switch (_a.label) {
          case 0:
            ...
            return [
              4 /*yield*/,
              figma.clientStorage.setAsync('apiSecret', secret),
            ];
          case 1:
            _a.sent();
